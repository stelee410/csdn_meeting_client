...
public class UserSubscribeController {
    ...
    @Operation(summary = "订阅标签", description = "用户订阅指定标签，订阅后可接收该标签下新会议的推送通知")
    ...
    @Operation(summary = "取消订阅标签", description = "取消订阅指定标签")
    ...
    @Operation(summary = "获取用户订阅列表", description = "获取当前用户已订阅的所有标签列表")
    ...
    @Operation(summary = "检查订阅状态", description = "检查当前用户是否已订阅指定标签")
    @GetMapping("/subscriptions/check")
    public ResponseEntity<ApiResponse<SubscriptionCheckDTO>> checkSubscription(
            @Parameter(description = "标签ID", required = true) @NotNull @RequestParam Long tagId,
            HttpServletRequest request) {
